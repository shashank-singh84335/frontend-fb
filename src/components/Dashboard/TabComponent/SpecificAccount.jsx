...
const [openModal, setOpenModal] = useState(false);
  ...
  useEffect(() => {
    ...
    const id=window.location.pathname.split("/")[2]
      const apiUrl = `${BASE_URL}/fb/account/${id}/`;
      try {
  ...
  return (
    <div className="flex flex-col gap-8 w-full min-h-screen">
      <div className="flex gap-4 items-center w-full">
        <div className="flex p-2 cursor-pointer rounded-md bg-white"
        onClick={() => {setState("account");navigate("/dashboard")}}
        >
          <ArrowLeft size={25} />
        </div>
        <div className="flex">
          <h1 className="font-bold text-3xl">{localStorage.getItem("account_name")}</h1>
        </div>
      </div>
      <div className="flex">
